...
void idLexer::Error( const char* str, ... )
{
	...
	{
		idLib::Warning( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	}
	else
	{
		idLib::Error( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	}
}
...
int idLexer::ReadToken( idToken* token )
{
	...
	if( !loaded )
	{
		idLib::Error( "idLexer::ReadToken: no file loaded" );
	}
...
int idLexer::LoadMemory( const char* ptr, int length, const char* name, int startLine )
{
	if( idLexer::loaded )
	{
		idLib::Error( "idLexer::LoadMemory: another script already loaded" );
	}
