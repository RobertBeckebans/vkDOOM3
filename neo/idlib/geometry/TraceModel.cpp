...
void idTraceModel::SetupCylinder( const idBounds& cylBounds, const int numSides )
{
	...
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCylinder: too many vertices\n" );
		n = MAX_TRACEMODEL_VERTS / 2;
	}
	if( n * 3 > MAX_TRACEMODEL_EDGES )
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCylinder: too many sides\n" );
		n = MAX_TRACEMODEL_EDGES / 3;
	}
	if( n + 2 > MAX_TRACEMODEL_POLYS )
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCylinder: too many polygons\n" );
...
void idTraceModel::SetupCone( const idBounds& coneBounds, const int numSides )
{
	...
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCone: too many vertices\n" );
		n = MAX_TRACEMODEL_VERTS - 1;
	}
	if( n * 2 > MAX_TRACEMODEL_EDGES )
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCone: too many edges\n" );
		n = MAX_TRACEMODEL_EDGES / 2;
	}
	if( n + 1 > MAX_TRACEMODEL_POLYS )
	{
		idLib::Printf( "WARNING: idTraceModel::SetupCone: too many polygons\n" );
...
void idTraceModel::SetupPolygon( const idVec3* v, const int count )
{
	...
	{
		idLib::Printf( "WARNING: idTraceModel::SetupPolygon: too many vertices\n" );
