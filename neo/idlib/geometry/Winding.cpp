...
bool idWinding::Check( bool print ) const
{
	...
	if( numPoints < 3 )
	{
		if( print )
		{
			idLib::Printf( "idWinding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if( area < 1.0f )
	{
		if( print )
		{
			idLib::Printf( "idWinding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for( i = 0; i < numPoints; i++ )
	{
		...
		for( j = 0; j < 3; j++ )
		{
			if( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD )
			{
				if( print )
				{
					idLib::Printf( "idWinding::Check: point %d outside world %c-axis: %f", i, 'X' + j, p1[j] );
				}
				return false;
			}
		}
		...
		if( d < -ON_EPSILON || d > ON_EPSILON )
		{
			if( print )
			{
				idLib::Printf( "idWinding::Check: point %d off plane.", i );
			}
			return false;
		}
		...
		if( dir.Length() < ON_EPSILON )
		{
			if( print )
			{
				idLib::Printf( "idWinding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for( j = 0; j < numPoints; j++ )
		{
			...
			if( d > edgedist )
			{
				if( print )
				{
					idLib::Printf( "idWinding::Check: non-convex." );
				}
				return false;
			}
		}
...
bool idFixedWinding::ReAllocate( int n, bool keep )
{
	...
	{
		idLib::Printf( "WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n" );
		return false;
	}
	return true;
}
