...
static drawSurf_t* R_AutospriteDeform( drawSurf_t* surf )
{
	...
	{
		idLib::Warning( "R_AutospriteDeform: shader had odd vertex count" );
		return NULL;
	}
	if( srcTri->numIndexes != ( srcTri->numVerts >> 2 ) * 6 )
	{
		idLib::Warning( "R_AutospriteDeform: autosprite had odd index count" );
		return NULL;
	}
...
static drawSurf_t* R_TubeDeform( drawSurf_t* surf )
{
	...
	if( srcTri->numVerts & 3 )
	{
		idLib::Error( "R_TubeDeform: shader had odd vertex count" );
	}
	if( srcTri->numIndexes != ( srcTri->numVerts >> 2 ) * 6 )
	{
		idLib::Error( "R_TubeDeform: autosprite had odd index count" );
	}
...
static drawSurf_t* R_FlareDeform( drawSurf_t* surf )
{
	...
	if( srcTri->numVerts != 4 || srcTri->numIndexes != 6 )
	{
		...
		idLib::Warning( "R_FlareDeform: not a single quad" );
		return NULL;
	}
...
static drawSurf_t* R_EyeballDeform( drawSurf_t* surf )
{
	...
	{
		idLib::Printf( "R_EyeballDeform: too many triangles in surface" );
		return NULL;
	}
	...
	if( numIslands != 3 )
	{
		idLib::Printf( "R_EyeballDeform: %i triangle islands\n", numIslands );
		return NULL;
	}
