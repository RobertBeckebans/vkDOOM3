...
int idMaterial::GetExpressionConstant( float f )
{
	...
	{
		idLib::Warning( "GetExpressionConstant: material '%s' hit MAX_EXPRESSION_REGISTERS", GetName() );
		SetMaterialFlag( MF_DEFAULTED );
		return 0;
	}
...
int idMaterial::GetExpressionTemporary()
{
	if( numRegisters >= MAX_EXPRESSION_REGISTERS )
	{
		idLib::Warning( "GetExpressionTemporary: material '%s' hit MAX_EXPRESSION_REGISTERS", GetName() );
		SetMaterialFlag( MF_DEFAULTED );
		return 0;
	}
...
expOp_t*	idMaterial::GetExpressionOp()
{
	...
	{
		idLib::Warning( "GetExpressionOp: material '%s' hit MAX_EXPRESSION_OPS", GetName() );
		SetMaterialFlag( MF_DEFAULTED );
...
int idMaterial::NameToSrcBlendMode( const idStr& name )
{
	...
	idLib::Warning( "unknown blend mode '%s' in material '%s'", name.c_str(), GetName() );
	SetMaterialFlag( MF_DEFAULTED );
...
void idMaterial::ParseVertexParm( idLexer& src, newShaderStage_t* newStage )
{
	...
	{
		idLib::Warning( "bad vertexParm number\n" );
		SetMaterialFlag( MF_DEFAULTED );
		return;
	}
...
void idMaterial::ParseFragmentMap( idLexer& src, newShaderStage_t* newStage )
{
	...
	{
		idLib::Warning( "bad fragmentMap number\n" );
		SetMaterialFlag( MF_DEFAULTED );
		return;
	}
...
void idMaterial::ParseStage( idLexer& src, const textureRepeat_t trpDefault )
{
	...
	{
		SetMaterialFlag( MF_DEFAULTED );
		idLib::Warning( "material '%s' exceeded %i stages", GetName(), MAX_SHADER_STAGES );
	}
	...
	while( 1 )
	{
		...
		if( !token.Icmp( "videomap" ) )
		{
			...
			if( !src.ReadToken( &token ) )
			{
				idLib::Warning( "missing parameter for 'videoMap' keyword in material '%s'", GetName() );
				continue;
			}
			bool loop = false;
			if( !token.Icmp( "loop" ) )
		...
		if( !token.Icmp( "soundmap" ) )
		{
			if( !src.ReadToken( &token ) )
			{
				idLib::Warning( "missing parameter for 'soundmap' keyword in material '%s'", GetName() );
				continue;
			}
		...
		if( !token.Icmp( "texGen" ) )
		{
			...
			{
				idLib::Warning( "bad texGen '%s' in material %s", token.c_str(), GetName() );
				SetMaterialFlag( MF_DEFAULTED );
			}
			continue;
		}
		...
		if( !token.Icmp( "rotate" ) )
		{
			...
			if( !table )
			{
				idLib::Warning( "no sinTable for rotate defined" );
				SetMaterialFlag( MF_DEFAULTED );
				return;
			}
			...
			if( !table )
			{
				idLib::Warning( "no cosTable for rotate defined" );
				SetMaterialFlag( MF_DEFAULTED );
				return;
			}
		...
		idLib::Warning( "unknown token '%s' in material '%s'", token.c_str(), GetName() );
		SetMaterialFlag( MF_DEFAULTED );
		return;
	}
...
void idMaterial::ParseMaterial( idLexer& src )
{
	...
	while( 1 )
	{
		...
		{
			idLib::Warning( "unknown general material parameter '%s' in '%s'", token.c_str(), GetName() );
			SetMaterialFlag( MF_DEFAULTED );
			return;
		}
	}
	...
	for( i = 0; i < numStages; i++ )
	{
		if( pd->parseStages[i].texture.texgen != TG_EXPLICIT )
		{
			...
			{
				idLib::Warning( "material '%s' has multiple stages with a texgen", GetName() );
				break;
			}
		}
	}
