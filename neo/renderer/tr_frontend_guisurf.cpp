...
/*
=================
idRenderSystemLocal::RenderGuiSurf

Create a texture space on the given surface and
call the GUI generator to create quads for it.
=================
*/
void idRenderSystemLocal::RenderGuiSurf( idUserInterface* gui, const drawSurf_t* drawSurf )
{
...
/*
================
idRenderSystemLocal::AddInGameGuis
================
*/
void idRenderSystemLocal::AddInGameGuis( const drawSurf_t* const drawSurfs[], const int numDrawSurfs )
{
	SCOPED_PROFILE_EVENT( "AddInGameGuis" );
	...
	for( int i = 0; i < numDrawSurfs; i++ )
	{
		...
		if( !PreciseCullSurface( drawSurf, ndcBounds ) )
		{
			...
			//	memcpy( guiShaderParms, shaderParms, sizeof( guiShaderParms ) );
