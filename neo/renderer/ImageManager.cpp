...
/*
==================
idImageManager::UploadImage
==================
*/
bool idImageManager::UploadImage( const char* imageName, const byte* data, int width, int height )
{
	idImage* image = GetImage( imageName );
	if( image == NULL )
	{
		return false;
	}
	image->UploadScratchImage( data, width, height );
	return true;
}
