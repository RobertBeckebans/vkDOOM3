...
void idMD5Mesh::ParseMesh( idLexer& parser, int numJoints, const idJointMat* joints )
{
	...
	for( int i = 0; i < texCoords.Num(); i++ )
	{
		...
		int	weights[256];
		const int weightsForVertex = numWeightsForVertex[ i ];
		...
		if( weightsForVertex > maxWeightsPerVert )
		{
			maxWeightsPerVert = weightsForVertex;
		}
		
		const int usedWeights = Min( MAX_VERTEX_WEIGHTS, weightsForVertex );
...
{
	idLib::Printf( "%s\n", name.c_str() );
	idLib::Printf( "Dynamic model.\n" );
	idLib::Printf( "Generated smooth normals.\n" );
	idLib::Printf( "    verts  tris weights material\n" );
	int	totalVerts = 0;
	int	totalTris = 0;
	...
	for( int i = 0; i < meshes.Num(); i++, mesh++ )
	{
		...
		idLib::Printf( "%2i: %5i %5i %s\n", i, mesh->NumVerts(), mesh->NumTris(), mesh->shader->GetName() );
	}
	idLib::Printf( "-----\n" );
	idLib::Printf( "%4i verts.\n", totalVerts );
	idLib::Printf( "%4i tris.\n", totalTris );
	idLib::Printf( "%4i joints.\n", joints.Num() );
}
...
void idRenderModelMD5::List() const
{
	...
	idLib::Printf( " %4ik %3i %4i %4i %s(MD5)", Memory() / 1024, meshes.Num(), totalVerts, totalTris, Name() );
...
idRenderModel* idRenderModelMD5::InstantiateDynamicModel( const renderEntity_t* ent, const viewDef_t* view, idRenderModel* cachedModel )
{
	...
	if( !ent->joints )
	{
		idLib::Printf( "idRenderModelMD5::InstantiateDynamicModel: NULL joints on renderEntity for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	}
	else if( ent->numJoints != joints.Num() )
	{
		idLib::Printf( "idRenderModelMD5::InstantiateDynamicModel: renderEntity has different number of joints than model for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	}
...
int idRenderModelMD5::NearestJoint( int surfaceNum, int a, int b, int c ) const
{
	...
	{
		idLib::Error( "idRenderModelMD5::NearestJoint: surfaceNum > meshes.Num()" );
	}
