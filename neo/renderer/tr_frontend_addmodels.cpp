...
bool R_IssueEntityDefCallback( idRenderEntity* def )
{
	...
	bool update;
	if( tr.m_viewDef != NULL )
	{
		update = def->parms.callback( &def->parms, &tr.m_viewDef->renderView );
	}
	else
	{
	...
	{
		idLib::Error( "R_IssueEntityDefCallback: dynamic entity callback didn't set model" );
	}
	...
	if( r_checkBounds.GetBool() )
	{
		...
		{
			idLib::Printf( "entity %i callback extended reference bounds\n", def->index );
		}
	}
...
This is also called by the game code for idRenderWorld::ModelTrace(), and idRenderWorld::Trace() which is bad for performance...
...
idRenderModel* R_EntityDefDynamicModel( idRenderEntity* def )
{
	...
	if( model == NULL )
	{
		idLib::Error( "R_EntityDefDynamicModel: NULL model" );
		return NULL;
	}
	...
	if( def->dynamicModel == NULL )
	{
		...
		def->cachedDynamicModel = model->InstantiateDynamicModel( &def->parms, tr.m_viewDef, def->cachedDynamicModel );
		...
		if( def->cachedDynamicModel != NULL && r_checkBounds.GetBool() )
		{
			...
			{
				idLib::Printf( "entity %i dynamic model exceeded reference bounds\n", def->index );
			}
		}
	...
	if( def->dynamicModel != NULL && model->DepthHack() != 0.0f && tr.m_viewDef != NULL )
	{
		idPlane eye, clip;
		idVec3 ndc;
		R_TransformModelToClip( def->parms.origin, tr.m_viewDef->worldSpace.modelViewMatrix, tr.m_viewDef->projectionMatrix, eye, clip );
	...
	else
	{
		...
		if( unlikely( renderEntity->referenceShader != NULL ) )
		{
			...
			renderEntity->referenceShader->EvaluateRegisters( refRegs, renderEntity->shaderParms,
					tr.m_viewDef->renderView.shaderParms,
					tr.m_viewDef->renderView.time[renderEntity->timeGroup] * 0.001f, renderEntity->referenceSound );
		...
		// process the shader expressions for conditionals / color / texcoords
		shader->EvaluateRegisters( regs, shaderParms, tr.m_viewDef->renderView.shaderParms,
								   tr.m_viewDef->renderView.time[renderEntity->timeGroup] * 0.001f, renderEntity->referenceSound );
	}
}
...
void R_AddSingleModel( viewEntity_t* vEntity )
{
	...
	idRenderEntity* entityDef = vEntity->entityDef;
	const renderEntity_t* renderEntity = &entityDef->parms;
	const idRenderWorld* world = entityDef->world;
		...
		for( viewLight_t* vLight = viewDef->viewLights; vLight != NULL; vLight = vLight->next )
		{
			...
			if( vLight->entityInteractionState != NULL )
			{
				...
				if( vLight->entityInteractionState[entityIndex] == viewLight_t::INTERACTION_YES )
				{
					...
					staticInteractions[numContactedLights] = world->m_interactionTable[vLight->lightDef->index * world->m_interactionTableWidth + entityIndex];
			...
			const idRenderLight* lightDef = vLight->lightDef;
	...
	for( int surfaceNum = 0; surfaceNum < model->NumSurfaces(); surfaceNum++ )
	{
		...
		if( tr.m_primaryRenderView.globalMaterial != NULL )
		{
			shader = tr.m_primaryRenderView.globalMaterial;
		}
		...
		if( r_checkBounds.GetBool() )
		{
			for( int j = 0; j < tri->numVerts; j++ )
			{
				...
				for( k = 0; k < 3; k++ )
				{
					...
					{
						idLib::Printf( "bad referenceBounds on %s:%s\n", entityDef->parms.hModel->Name(), shader->GetName() );
						break;
					}
				}
				if( k != 3 )
		...
		if( surfaceDirectlyVisible )
		{
			...
			baseDrawSurf = ( drawSurf_t* )renderSystem->FrameAlloc( sizeof( *baseDrawSurf ), FRAME_ALLOC_DRAW_SURFACE );
			...
			if( shaderDeform != DFRM_NONE )
			{
				...
				if( deformDrawSurf != NULL )
				{
					...
					for( drawSurf_t* current = deformDrawSurf, * next = NULL; current != NULL; current = next )
					{
						next = current->nextOnLight;
						
						current->linkChain = NULL;
						current->nextOnLight = vEntity->drawSurfs;
						vEntity->drawSurfs = current;
					}
				}
			}
		...
		for( int contactedLight = 0; contactedLight < numContactedLights; contactedLight++ )
		{
			...
			if( addInteractions && surfaceDirectlyVisible && shader->ReceivesLighting() )
			{
				...
				if( surfInter == NULL || surfInter->lightTrisIndexCache > 0 )
				{
					...
					drawSurf_t* lightDrawSurf = ( drawSurf_t* )renderSystem->FrameAlloc( sizeof( *lightDrawSurf ), FRAME_ALLOC_DRAW_SURFACE );
					...
					else
					{
						...
						if( r_cullDynamicLightTriangles.GetBool() )
						{
							...
							if( vertexCache.CacheIsCurrent( lightIndexCache ) )
							{
								...
								dynamicShadowParms = ( dynamicShadowVolumeParms_t* )renderSystem->FrameAlloc( sizeof( dynamicShadowParms[0] ), FRAME_ALLOC_SHADOW_VOLUME_PARMS );
			...
			drawSurf_t* shadowDrawSurf = ( drawSurf_t* )renderSystem->FrameAlloc( sizeof( *shadowDrawSurf ), FRAME_ALLOC_DRAW_SURFACE );
			...
			if( surfInter != NULL )
			{
				...
				{
					staticShadowVolumeParms_t* staticShadowParms = ( staticShadowVolumeParms_t* )renderSystem->FrameAlloc( sizeof( staticShadowParms[0] ), FRAME_ALLOC_SHADOW_VOLUME_PARMS );
...
/*
===================
idRenderSystemLocal::AddModels

The end result of running this is the addition of drawSurf_t to the
viewDef->drawSurfs[] array and light link chains, along with
...
void idRenderSystemLocal::AddModels()
{
	...
	m_viewDef->viewEntitys = R_SortViewEntities( m_viewDef->viewEntitys );
	...
	if( r_useParallelAddModels.GetBool() )
	{
		...
		{
			m_frontEndJobList->AddJob( ( jobRun_t )R_AddSingleModel, vEntity );
		}
	...
	m_viewDef->numDrawSurfs = 0;	// clear the ambient surface list
	m_viewDef->maxDrawSurfs = 0;	// will be set to INITIAL_DRAWSURFS on R_LinkDrawSurfToView
	...
	for( viewEntity_t* vEntity = m_viewDef->viewEntitys; vEntity != NULL; vEntity = vEntity->next )
	{
		for( drawSurf_t* ds = vEntity->drawSurfs; ds != NULL; )
		{
			...
			{
				R_LinkDrawSurfToView( ds, m_viewDef );
			}
			else
			{
