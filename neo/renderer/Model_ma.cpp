...
#define MA_VERBOSE( x ) { if ( maGlobal.verbose ) { idLib::Printf x ; } }
...
bool MA_ParseTransform( idParser& parser )
{
	...
	transform = ( maTransfort* )MeAlloc( sizeof( maTransfort ), TAG_MODEL );
	memset( transform, 0, sizeof( maTransfort ) );
...
bool MA_ParseVertexTransforms( idParser& parser, maAttribHeader_t* header )
{
	...
	if( !pMesh->vertTransforms )
	{
		...
		pMesh->vertTransforms = ( idVec4* )MeAlloc( sizeof( idVec4 ) * pMesh->numVertTransforms, TAG_MODEL );
...
bool MA_ParseEdge( idParser& parser, maAttribHeader_t* header )
{
	...
	if( !pMesh->edges )
	{
		...
		pMesh->edges = ( idVec3* )MeAlloc( sizeof( idVec3 ) * pMesh->numEdges, TAG_MODEL );
	}
...
bool MA_ParseNormal( idParser& parser, maAttribHeader_t* header )
{
	...
	if( !pMesh->normals )
	{
		...
		pMesh->normals = ( idVec3* )MeAlloc( sizeof( idVec3 ) * pMesh->numNormals, TAG_MODEL );
	}
...
bool MA_ParseFace( idParser& parser, maAttribHeader_t* header )
{
	...
	if( !pMesh->faces )
	{
		...
		pMesh->faces = ( maFace_t* )MeAlloc( sizeof( maFace_t ) * pMesh->numFaces, TAG_MODEL );
	}
...
bool MA_ParseColor( idParser& parser, maAttribHeader_t* header )
{
	...
	if( !pMesh->colors )
	{
		...
		pMesh->colors = ( byte* )MeAlloc( sizeof( byte ) * pMesh->numColors * 4, TAG_MODEL );
	}
...
void MA_ParseMesh( idParser& parser )
{
	...
	maObject_t*	object;
	object = ( maObject_t* )MeAlloc( sizeof( maObject_t ), TAG_MODEL );
	...
	while( parser.ReadToken( &token ) )
	{
		...
		if( !token.Icmp( "setAttr" ) )
		{
			maAttribHeader_t attrHeader;
			MA_ParseAttribHeader( parser, &attrHeader );
			
			if( strstr( attrHeader.name, ".vt" ) )
			{
				MA_ParseVertex( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".ed" ) )
			{
				MA_ParseEdge( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".pt" ) )
			{
				MA_ParseVertexTransforms( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".n" ) )
			{
				MA_ParseNormal( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".fc" ) )
			{
				MA_ParseFace( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".clr" ) )
			{
				MA_ParseColor( parser, &attrHeader );
			}
			else if( strstr( attrHeader.name, ".uvst" ) )
			{
				MA_ParseTVert( parser, &attrHeader );
			}
			else
			{
				parser.SkipRestOfLine();
	...
	while( parser.ReadToken( &token ) )
	{
		...
		if( !token.Icmp( "setAttr" ) )
		{
			...
			if( strstr( attribHeader.name, ".ftn" ) )
			{
				...
				fileNode = ( maFileNode_t* )MeAlloc( sizeof( maFileNode_t ), TAG_MODEL );
...
void MA_ParseMaterialNode( idParser& parser )
{
	...
	matNode = ( maMaterialNode_t* )MeAlloc( sizeof( maMaterialNode_t ), TAG_MODEL );
...
int MA_AddMaterial( const char* materialName )
{
	...
	if( destNode )
	{
		...
		if( matNode && matNode->file )
		{
			...
			material = ( maMaterial_t* )MeAlloc( sizeof( maMaterial_t ), TAG_MODEL );
...
void MA_ApplyTransformation( maModel_t* model )
{
	...
	for( int i = 0; i < model->objects.Num(); i++ )
	{
		...
		maTransfort* transform = mesh->transform;
