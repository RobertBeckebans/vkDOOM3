...
#define SET_COMP( index, id, hsamp, vsamp, quant, dctbl, actbl )  \
	...
	switch( colorspace )
	{
		case JCS_GRAYSCALE:
			...
			cinfo->nucomponents = 1;
			/* JFIF specifies component ID 1 */
			SET_COMP( 0, 1, 1, 1, 0, 0, 0 );
			break;
		case JCS_RGB:
			cinfo->write_Adobe_marker = TRUE;/* write Adobe marker to flag RGB */
			cinfo->nucomponents = 3;
		...
		case JCS_UNKNOWN:
			...
			if( ( cinfo->nucomponents < 1 ) || ( cinfo->nucomponents > MAX_COMPONENTS ) )
			{
 ...
 * cinfo->nucomponents and cinfo->jpeg_color_space must be correct.
 */

GLOBAL void
jpeg_simple_progression( j_compress_ptr cinfo )
{
	int ncomps = cinfo->nucomponents;
	int nscans;
