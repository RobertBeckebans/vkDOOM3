...
#define VERBOSE( x ) { if ( ase.verbose ) { idLib::Printf x ; } }
...
static void ASE_KeyMESH_VERTEX_LIST( const char* token )
{
	...
	if( !strcmp( token, "*MESH_VERTEX" ) )
	{
		...
		{
			idLib::Error( "ase.currentVertex >= pMesh->numVertexes" );
		}
	}
	else
	{
		idLib::Error( "Unknown token '%s' while parsing MESH_VERTEX_LIST", token );
	}
}
...
static void ASE_KeyMESH_FACE_LIST( const char* token )
{
	...
	if( !strcmp( token, "*MESH_FACE" ) )
	{
		...
		/*
				...
				{
					idLib::Error( "No *MESH_MTLID found for face!" );
				}
		*/
	...
	{
		idLib::Error( "Unknown token '%s' while parsing MESH_FACE_LIST", token );
	}
}
...
static void ASE_KeyTFACE_LIST( const char* token )
{
	...
	{
		idLib::Error( "Unknown token '%s' in MESH_TFACE", token );
	}
}
...
static void ASE_KeyCFACE_LIST( const char* token )
{
	...
	{
		idLib::Error( "Unknown token '%s' in MESH_CFACE", token );
	}
}
...
static void ASE_KeyMESH_TVERTLIST( const char* token )
{
	...
	if( !strcmp( token, "*MESH_TVERT" ) )
	{
		...
		{
			idLib::Error( "ase.currentVertex > pMesh->numTVertexes" );
		}
	}
	else
	{
		idLib::Error( "Unknown token '%s' while parsing MESH_TVERTLIST", token );
	}
}
...
static void ASE_KeyMESH_CVERTLIST( const char* token )
{
	...
	if( !strcmp( token, "*MESH_VERTCOL" ) )
	{
		...
		{
			idLib::Error( "ase.currentVertex > pMesh->numCVertexes" );
		}
	}
	else
	{
		idLib::Error( "Unknown token '%s' while parsing MESH_CVERTLIST", token );
	}
}
...
static void ASE_KeyMESH_NORMALS( const char* token )
{
	...
	if( !strcmp( token, "*MESH_FACENORMAL" ) )
	{
		...
		{
			idLib::Error( "MESH_NORMALS face index out of range: %i", num );
		}
		
		if( num != ase.currentFace )
		{
			idLib::Error( "MESH_NORMALS face index != currentFace" );
		}
	...
	else if( !strcmp( token, "*MESH_VERTEXNORMAL" ) )
	{
		...
		{
			idLib::Error( "MESH_NORMALS vertex index out of range: %i", num );
		}
		...
		if( v >= 3 )
		{
			idLib::Error( "MESH_NORMALS vertex index doesn't match face" );
			return;
		}
...
static void ASE_KeyMESH( const char* token )
{
	...
	else if( !strcmp( token, "*MESH_NUMTVFACES" ) )
	{
		...
		{
			idLib::Error( "MESH_NUMTVFACES != MESH_NUMFACES" );
		}
	}
	else if( !strcmp( token, "*MESH_NUMCVFACES" ) )
	{
		...
		if( pMesh->numTVFaces != pMesh->numFaces )
		{
			idLib::Error( "MESH_NUMCVFACES != MESH_NUMFACES" );
		}
	}
	...
	else if( !strcmp( token, "*MESH_TFACELIST" ) )
	{
		if( !pMesh->faces )
		{
			idLib::Error( "*MESH_TFACELIST before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
	...
	else if( !strcmp( token, "*MESH_CFACELIST" ) )
	{
		if( !pMesh->faces )
		{
			idLib::Error( "*MESH_CFACELIST before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
	...
	else if( !strcmp( token, "*MESH_NORMALS" ) )
	{
		if( !pMesh->faces )
		{
			idLib::Warning( "*MESH_NORMALS before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
...
static void ASE_KeyMESH_ANIMATION( const char* token )
{
	...
	{
		idLib::Error( "Unknown token '%s' while parsing MESH_ANIMATION", token );
	}
}
