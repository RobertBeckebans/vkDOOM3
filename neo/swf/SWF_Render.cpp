...
void idSWF::RenderEditText( idRenderSystem* gui, idSWFTextInstance* textInstance, const swfRenderState_t& renderState, int time, bool isSplitscreen )
{
	...
	if( inputField )
	{
		if( textLines.Num() > 0 )
		{
			...
			bool endFound = false;
			int idx = startCheckIndex;
			while( idx < line.Length() )
			{
				uint32 tc = line.UTF8Char( idx );
				scaledGlyphInfo_t glyph;
				fontInfo->GetScaledGlyph( glyphScale, tc, glyph );
				float glyphSkip = glyph.xSkip;
				if( textInstance->HasStroke() )
				...
				{
					if( cursorPos > idx && cursorPos != endCharacter )
					{
	...
	for( int t = 0; t < textLines.Num(); t++ )
	{
		...
		while( i < lastChar )
		{
			...
			{
				idVec2 _topl = matrix.Transform( idVec2( x, y ) );
				idVec2 _topr = matrix.Transform( idVec2( x + glyphSkip, y ) );
				idVec2 _br = matrix.Transform( idVec2( x + glyphSkip, y + linespacing ) );
				idVec2 _bl = matrix.Transform( idVec2( x, y + linespacing ) );
				gui->SetColor( selColor );
				DrawStretchPic(
					idVec4( _topl.x, _topl.y, 0, 0 ),
					idVec4( _topr.x, _topr.y, 1, 0 ),
					idVec4( _br.x, _br.y, 1, 1 ),
					idVec4( _bl.x, _bl.y, 0, 1 ), white );
