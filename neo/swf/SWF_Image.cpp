...
void idSWF::WriteSwfImageAtlas( const char* filename )
{
	...
	for( int i = 0 ; i < packImages.Num() ; i++ )
	{
		...
		int outX = outputPositions[i].x;
		int outY = outputPositions[i].y;
...
//			minV[0], maxV[0], minV[1], maxV[1], minV[2], maxV[2], minV[3], maxV[3] );
		...
		assert( ( outX + blockWidth ) * 4 <= atlasWidth );
		assert( ( outY + blockHeight ) * 4 <= atlasHeight );
		...
		for( int dstY = 0; dstY < blockHeight << 2; dstY++ )
		{
			...
			for( int dstX = 0 ; dstX < blockWidth << 2 ; dstX++ )
			{
				...
				( ( int* )swfAtlas.Ptr() )[( outY + dstY ) * atlasWidth + ( outX + dstX ) ] =
		...
		entry->imageAtlasOffset.x = outX + 1;
		entry->imageAtlasOffset.y = outY + 1;
		for( int j = 0; j < 4; j++ )
		{
