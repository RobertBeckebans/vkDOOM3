...
void idWindow::Transition()
{
	...
	for( i = 0; i < c; i++ )
	{
		...
		else
		{
			...
			{
				idLib::Warning( "Invalid transitional data for window %s in gui %s", GetName(), gui->GetSourceFile() );
			}
		}
	}
...
bool idWindow::ParseInternalVar( const char* _name, idTokenParser* src )
{
	...
	{
		idLib::Warning( "play encountered during gui parse.. see Robert\n" );
		idStr playStr;
...
int idWindow::ExpressionConstant( float f )
{
	...
	{
		idLib::Warning( "expressionConstant: gui %s hit MAX_EXPRESSION_REGISTERS", gui->GetSourceFile() );
		return 0;
	}
...
int idWindow::ExpressionTemporary()
{
	if( expressionRegisters.Num() == MAX_EXPRESSION_REGISTERS )
	{
		idLib::Warning( "expressionTemporary: gui %s hit MAX_EXPRESSION_REGISTERS", gui->GetSourceFile() );
		return 0;
	}
...
wexpOp_t* idWindow::ExpressionOp()
{
	...
	{
		idLib::Warning( "expressionOp: gui %s hit MAX_EXPRESSION_OPS", gui->GetSourceFile() );
		return &ops[0];
	}
	wexpOp_t wop;
	...
	for( i = 0 ; i < oc ; i++ )
	{
		...
		switch( op->opType )
		{
			...
			case WOP_TYPE_DIVIDE:
				...
				{
					idLib::Warning( "Divide by zero in window '%s' in %s", GetName(), gui->GetSourceFile() );
...
void idWindow::ReadSaveGameString( idStr& string, idFile* savefile )
{
	...
	if( len < 0 )
	{
		idLib::Warning( "idWindow::ReadSaveGameString: invalid length" );
	}
