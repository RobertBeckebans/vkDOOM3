...
void idChoiceWindow::InitVars( )
{
	if( cvarStr.Length() )
	{
		...
		if( !cvar )
		{
			idLib::Warning( "idChoiceWindow::InitVars: gui '%s' window '%s' references undefined cvar '%s'", gui->GetSourceFile(), name.c_str(), cvarStr.c_str() );
			return;
		}
...
void idChoiceWindow::UpdateChoicesAndVals()
{
	...
	if( choiceVals.Length() && latchedVals.Icmp( choiceVals ) )
	{
		...
		{
			idLib::Warning( "idChoiceWindow:: gui '%s' window '%s' has value count unequal to choices count", gui->GetSourceFile(), name.c_str() );
		}
