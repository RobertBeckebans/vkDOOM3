...
void Script_Transition( idWindow* window, idList<idGSWinVar, TAG_OLD_UI>* src )
{
	...
	if( src->Num() >= 4 )
	{
		...
		{
			//
			idLib::Warning( "Bad transition in gui %s in window %s\n", window->GetGui()->GetSourceFile(), window->GetName() );
			return;
		}
...
void idGuiScript::FixupParms( idWindow* win )
{
	if( handler == &Script_Set )
	{
		...
		idWinStr* vstr = dynamic_cast<idWinStr*>( parms[0].var );
		assert( vstr );
		idWinVar* dest = win->GetWinVarByName( *vstr, true );
		if( dest )
		{
			delete parms[0].var;
			parms[0].var = dest;
		...
		else if( idStr::Icmp( vstr->c_str(), "cmd" ) == 0 )
		{
			precacheSounds = true;
		}
	...
	else if( handler == &Script_Transition )
	{
		if( parms.Num() < 4 )
		{
			idLib::Warning( "Window %s in gui %s has a bad transition definition", win->GetName(), win->GetGui()->GetSourceFile() );
		}
		...
		{
			idLib::Warning( "Window %s in gui %s: a transition does not have a valid destination var %s", win->GetName(), win->GetGui()->GetSourceFile(), str->c_str() );
		}
