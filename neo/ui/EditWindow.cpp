...
void idEditWindow::InitCvar( )
{
	if( cvarStr[0] == '\0' )
	{
		...
		{
			idLib::Warning( "idEditWindow::InitCvar: gui '%s' window '%s' has an empty cvar string", gui->GetSourceFile(), name.c_str() );
		}
		cvar = NULL;
		return;
	}
	...
	if( !cvar )
	{
		idLib::Warning( "idEditWindow::InitCvar: gui '%s' window '%s' references undefined cvar '%s'", gui->GetSourceFile(), name.c_str(), cvarStr.c_str() );
		return;
	}
}
