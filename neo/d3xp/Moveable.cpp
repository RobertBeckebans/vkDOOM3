...
void idExplodingBarrel::Killed( idEntity* inflictor, idEntity* attacker, int damage, const idVec3& dir, int location )
{
	...
	while( kv != NULL )
	{
		...
		if( debris_args )
		{
			...
			idDebris* debris;
			//if ( first ) {
			direction = physicsObj.GetAxis()[1];
			//	first = false;
			//} else {
			direction.x += gameLocal.random.CRandomFloat() * 4.0f;
			direction.y += gameLocal.random.CRandomFloat() * 4.0f;
			//direction.z = gameLocal.random.RandomFloat() * 8.0f;
			//}
			direction.Normalize();
			...
			debris->Create( this, physicsObj.GetOrigin(), direction.ToMat3() );
			debris->Launch();
