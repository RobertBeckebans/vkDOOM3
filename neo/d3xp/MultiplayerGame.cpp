...
void idMultiplayerGame::UpdateScoreboard( idMenuHandler_Scoreboard* scoreboard, idPlayer* owner )
{
	...
	for( int i = 0; i < MAX_CLIENTS; ++i )
	{
		...
		if( i < gameLocal.numClients )
		{
			...
			{
				if( info.team == scoreboardInfo[ j ].team )
				{
					if( info.score > scoreboardInfo[ j ].score )
					{
						scoreboardInfo.Insert( info, j );
						added = true;
						break;
					}
				}
				else if( info.team < scoreboardInfo[ j ].team )
				{
					scoreboardInfo.Insert( info, j );
					added = true;
					break;
...
void idMultiplayerGame::DropWeapon_f( const idCmdArgs& args )
{
	if( !common->IsMultiplayer() )
	{
		idLib::Printf( "clientDropWeapon: only valid in multiplayer\n" );
		return;
	}
	idBitMsg outMsg;
...
void idMultiplayerGame::VoiceChat( const idCmdArgs& args, bool team )
{
	...
	if( args.Argc() != 2 )
	{
		idLib::Printf( "clientVoiceChat: bad args\n" );
		return;
	}
	// throttle
	...
	if( !keyval )
	{
		idLib::Printf( "Voice command not found: %s\n", voc );
		return;
	}
