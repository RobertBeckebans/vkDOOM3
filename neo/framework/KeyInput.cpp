...
void Key_Unbind_f( const idCmdArgs& args )
{
	...
	if( args.Argc() != 2 )
	{
		idLib::Printf( "unbind <key> : remove commands from a key\n" );
		return;
	}
	...
	if( b == -1 )
	{
		...
		{
			idLib::Printf( "\"%s\" isn't a valid key\n", args.Argv( 1 ) );
		}
	}
	else
	{
...
void Key_Bind_f( const idCmdArgs& args )
{
	...
	if( c < 2 )
	{
		idLib::Printf( "bind <key> [command] : attach a command to a key\n" );
		return;
	}
	b = idKeyInput::StringToKeyNum( args.Argv( 1 ) );
	if( b == -1 )
...
void Key_BindUnBindTwo_f( const idCmdArgs& args )
{
	...
	if( c < 3 )
	{
		idLib::Printf( "bindunbindtwo <keynum> [command]\n" );
		return;
	}
