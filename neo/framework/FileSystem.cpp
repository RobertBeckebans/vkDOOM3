...
void idFileSystemLocal::WriteResourcePacks()
{
	...
	{
		idStr filePath = "maps/";
		filePath += manifestFiles[ i ];
		idFileManifest manifest;
		if( manifest.LoadManifest( filePath ) )
		{
			//manifest.Print();
	...
	{
		idStr filePath = "maps/";
		filePath += preloadFiles[ i ];
		if( filePath.Find( "_startup", false ) >= 0 )
		{
			continue;
		}
...
void idFileSystemLocal::DirTree_f( const idCmdArgs& args )
{
	...
	if( args.Argc() < 2 || args.Argc() > 3 )
	{
		idLib::Printf( "usage: dirtree <directory> [extension]\n" );
		return;
	}
...
void idFileSystemLocal::WriteResourceFile_f( const idCmdArgs& args )
{
	if( args.Argc() != 2 )
	{
		idLib::Printf( "Usage: writeResourceFile <manifest file>\n" );
		return;
	}
...
void idFileSystemLocal::UpdateResourceFile_f( const idCmdArgs& args )
{
	if( args.Argc() < 3 )
	{
		idLib::Printf( "Usage: updateResourceFile <resource file> <files>\n" );
		return;
	}
...
void idFileSystemLocal::ExtractResourceFile_f( const idCmdArgs& args )
{
	if( args.Argc() < 3 )
	{
		idLib::Printf( "Usage: extractResourceFile <resource file> <outpath> <copysound>\n" );
		return;
	}
...
void idFileSystemLocal::TouchFile_f( const idCmdArgs& args )
{
	...
	if( args.Argc() != 2 )
	{
		idLib::Printf( "Usage: touchFile <file>\n" );
		return;
	}
...
void idFileSystemLocal::TouchFileList_f( const idCmdArgs& args )
{
	...
	if( args.Argc() != 2 )
	{
		idLib::Printf( "Usage: touchFileList <filename>\n" );
		return;
	}
...
int idFileSystemLocal::AddResourceFile( const char* resourceFileName )
{
	...
	if( rc->Init( resourceFile, resourceFiles.Num() ) )
	{
		...
		idLib::Printf( "Loaded resource file %s\n", resourceFile.c_str() );
	...
	if( searchFlags & FSFLAG_SEARCH_DIRS )
	{
		for( int sp = searchPaths.Num() - 1; sp >= 0; sp-- )
		{
			...
			if( allowCopyFiles )
			{
				...
				if( fs_buildResources.GetBool() )
				{
					idStrStatic< MAX_OSPATH > relPath = OSPathToRelativePath( copypath );
					relPath.BackSlashesToSlashes();
					relPath.ToLower();

					if( IsSoundSample( relPath ) )
					{
						idStrStatic< MAX_OSPATH > samplePath = relPath;
					...
					else if( relPath.Icmpn( "guis/", 5 ) == 0 )
					{
						...
						for( int i = 0; i < Sys_NumLangs(); i++ )
						{
							...
							if( idStr::Icmp( lang, ID_LANG_ENGLISH ) == 0 )
							{
								fileManifest.Append( relPath );
								continue;
							}
							idStrStatic< MAX_OSPATH > guiPath = relPath;
...
idFile* idFileSystemLocal::OpenFileWrite( const char* relativePath, const char* basePath )
{
	...
	if( fs_debug.GetInteger() )
	{
		idLib::Printf( "idFileSystem::OpenFileWrite: %s\n", OSpath.c_str() );
	}
...
idFile* idFileSystemLocal::OpenExplicitFileRead( const char* OSPath )
{
	...
	if( fs_debug.GetInteger() )
	{
		idLib::Printf( "idFileSystem::OpenExplicitFileRead: %s\n", OSPath );
	}
...
idFile* idFileSystemLocal::OpenExplicitFileWrite( const char* OSPath )
{
	...
	if( fs_debug.GetInteger() )
	{
		idLib::Printf( "idFileSystem::OpenExplicitFileWrite: %s\n", OSPath );
	}
...
idFile* idFileSystemLocal::OpenFileAppend( const char* relativePath, bool sync, const char* basePath )
{
	...
	if( fs_debug.GetInteger() )
	{
		idLib::Printf( "idFileSystem::OpenFileAppend: %s\n", OSpath.c_str() );
	}
