...
idCVar con_speed( "con_speed", "3", CVAR_SYSTEM, "speed at which the console moves up and down" );
idCVar con_notifyTime( "con_notifyTime", "3", CVAR_SYSTEM, "time messages are displayed onscreen when console is pulled up" );
#ifdef DEBUG
idCVar con_noPrint( "con_noPrint", "0", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT, "print on the console but not onscreen when console is pulled up" );
#else
idCVar con_noPrint( "con_noPrint", "1", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT, "print on the console but not onscreen when console is pulled up" );
#endif
...
static void Con_Dump_f( const idCmdArgs& args )
{
	if( args.Argc() != 2 )
	{
		idLib::Printf( "usage: conDump <filename>\n" );
		return;
	}
	...
	idLib::Printf( "Dumped console text to %s.\n", fileName.c_str() );
...
void idConsoleLocal::Dump( const char* fileName )
{
	...
	if( !f )
	{
		idLib::Warning( "couldn't open %s", fileName );
		return;
	}
...
void idConsoleLocal::KeyDownEvent( int key )
{
	...
	if( key == K_ENTER || key == K_KP_ENTER )
	{
		...
		common->UpdateScreen(); // force an update, because the command
		// may take some time
		return;
	}
