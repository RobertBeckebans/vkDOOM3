...
bool idAFVector::Finish( const char* fileName, const getJointTransform_t GetJointTransform, const idJointMat* frame, void* model ) const
{
	...
	switch( type )
	{
		...
		case idAFVector::VEC_JOINT:
		{
			...
			{
				idLib::Warning( "invalid joint %s in joint() in '%s'", joint1.c_str(), fileName );
				vec.Zero();
			}
			break;
		}
		case idAFVector::VEC_BONECENTER:
		{
			...
			{
				idLib::Warning( "invalid joint %s in bonecenter() in '%s'", joint1.c_str(), fileName );
				start.Zero();
			}
			if( !GetJointTransform( model, frame, joint2, end, axis ) )
			{
				idLib::Warning( "invalid joint %s in bonecenter() in '%s'", joint2.c_str(), fileName );
				end.Zero();
			}
			vec = ( start + end ) * 0.5f;
			break;
		...
		case idAFVector::VEC_BONEDIR:
		{
			if( !GetJointTransform( model, frame, joint1, start, axis ) )
			{
				idLib::Warning( "invalid joint %s in bonedir() in '%s'", joint1.c_str(), fileName );
				start.Zero();
			}
			if( !GetJointTransform( model, frame, joint2, end, axis ) )
			{
				idLib::Warning( "invalid joint %s in bonedir() in '%s'", joint2.c_str(), fileName );
				end.Zero();
			}
			vec = ( end - start );
			break;
