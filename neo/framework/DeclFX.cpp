...
void idDeclFX::Print() const
{
	...
	idLib::Printf( "%d events\n", list->events.Num() );
	for( int i = 0; i < list->events.Num(); i++ )
	{
		switch( list->events[i].type )
		{
			case FX_LIGHT:
				idLib::Printf( "FX_LIGHT %s\n", list->events[i].data.c_str() );
				break;
			case FX_PARTICLE:
				idLib::Printf( "FX_PARTICLE %s\n", list->events[i].data.c_str() );
				break;
			case FX_MODEL:
				idLib::Printf( "FX_MODEL %s\n", list->events[i].data.c_str() );
				break;
			case FX_SOUND:
				idLib::Printf( "FX_SOUND %s\n", list->events[i].data.c_str() );
				break;
			case FX_DECAL:
				idLib::Printf( "FX_DECAL %s\n", list->events[i].data.c_str() );
				break;
			case FX_SHAKE:
				idLib::Printf( "FX_SHAKE %s\n", list->events[i].data.c_str() );
				break;
			case FX_ATTACHLIGHT:
				idLib::Printf( "FX_ATTACHLIGHT %s\n", list->events[i].data.c_str() );
				break;
			case FX_ATTACHENTITY:
				idLib::Printf( "FX_ATTACHENTITY %s\n", list->events[i].data.c_str() );
				break;
			case FX_LAUNCH:
				idLib::Printf( "FX_LAUNCH %s\n", list->events[i].data.c_str() );
				break;
			case FX_SHOCKWAVE:
				idLib::Printf( "FX_SHOCKWAVE %s\n", list->events[i].data.c_str() );
				break;
		}
	}
}
